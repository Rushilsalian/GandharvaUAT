...
export async function registerRoutes(app: Express): Promise<Server> {
      ...
      for (const clientData of validClients) {
        ...
        try {
          ...
          const newClient = {
            ...
            pincode: clientData.pincode && !isNaN(parseInt(clientData.pincode)) ? parseInt(clientData.pincode) : null,
            referenceId: clientData.reference_code ? (await storage.getMstClientByCode(clientData.reference_code))?.clientId || null : null,
            openingInvestment: clientData.opening_investment || null,
  ...
  // Dashboard API endpoints - role-based access (moved to enhancedDashboardRoutes.ts)

  // Dashboard trends endpoint (moved to enhancedDashboardRoutes.ts)
  ...
  // Enhanced dashboard endpoints (moved to enhancedDashboardRoutes.ts)

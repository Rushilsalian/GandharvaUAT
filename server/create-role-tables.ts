...
async function createRoleTables() {
  ...
  await pool.execute(moduleTable);
  console.log('mst_module table created');
  
  await pool.execute(roleRightTable);
  console.log('mst_role_right table created');
}

createRoleTables().catch(console.error);
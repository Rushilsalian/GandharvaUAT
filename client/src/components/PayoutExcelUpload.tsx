...
interface PayoutExcelUploadProps {
  onUploadComplete?: (result: UploadResult) => void;
}

export function PayoutExcelUpload({ onUploadComplete }: PayoutExcelUploadProps) {
  ...
  const downloadSample = () => {
    const sampleData = [
      { client_code: 'CLI001', date: '15-01-2024', amount: 5000, remark: 'Monthly payout' },
      { client_code: 'CLI002', date: '16-01-2024', amount: 7500, remark: 'Quarterly payout' },
      { client_code: 'CLI003', date: '17-01-2024', amount: 10000, remark: '' }
    ];
    
    const ws = XLSX.utils.json_to_sheet(sampleData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Payout Sample');
    XLSX.writeFile(wb, 'payout_sample.xlsx');
  };
    ...
    <Card className="w-full">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          Excel Upload - Payout Transactions
        </CardTitle>
        <CardDescription>
          Upload Excel file with payout transaction data. Indicator ID: 2 (Payout)
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        {result && (
          <Alert variant={result.errors.length === 0 ? "default" : "destructive"}>
            ...
            <AlertDescription>
              ...
              <div className="mt-2">
                <p>Successfully processed: {result.success} payout transactions</p>
                {result.errors.length > 0 && (
                  <div className="mt-2">
        ...
        <div className="flex gap-2">
          <Button
            ...
            className="flex-1"
          >
            {uploading ? 'Uploading...' : 'Upload Payout Transactions'}
          </Button>
          {(file || result) && (
